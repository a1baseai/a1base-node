...
A powerful and easy-to-use Node.js client for interacting with the A1Base API.

## Features

- Send individual messages via WhatsApp or Telegram
...
```javascript
try {
  const groupDetails = await client.getChatGroupDetails('accountId', 'threadId');
...
```javascript
try {
  const threads = await client.getAllThreadsByNumber('accountId', 'phoneNumber');
